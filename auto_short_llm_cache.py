...
def sample_random_frame(video: str, out_jpg: str) -> float:
    ...
    # -update 1 helps when target filename lacks sequence pattern on some ffmpeg builds
...
def blur_boxes_chain(src_label: str, boxes: List[dict]) -> str:
    ...
    BLUR = "9:1:9:1"
    chain = f"[{src_label}]split=1[s0]"
    prev = "s0"
    for i, b in enumerate(wm, start=1):
        x, y, w, h = b["x"], b["y"], b["w"], b["h"]
        chain += (
            f";[{prev}]split=2[{prev}a][for{i}]"
...
def compose_with_boxes(video_in: str, bg_path: str, out_mp4: str, boxes: List[dict]):
    """Кропим боковые панели (если есть) и кладём 16:9 по центру 9:16 фона. Сейчас без блюра."""
...
# ---------- CLI main (optional) ----------
def main():
    ap=argparse.ArgumentParser(description="YT → cut (cache) → LLM detect → crop → overlay 16:9 center")
